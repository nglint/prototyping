...
cout<<s.first<<'-'<<s.second<<endl;
 ...
 adalm::adalm() {   mp[0]="iio:device0",mp[1]="iio:device1",mp[2]="iio:device2",mp[3]="iio:device3";
...
int  adalm::intcontrol(char *ip)
{
 con = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    ...
    int adalm::intread(char *ip,int buff)
...
int adalm::intwrite(char *ip,int buff)
...
da=da+" 00000003 CYCLIC\r\n";
    ...
    struct sockaddr_in serverAddr;
    serverAddr.sin_family = AF_INET;
    serverAddr.sin_port = htons(30431);
 ...
 recv(wr,r , 4, 0);
  ...
  int adalm::writebuff(char *recvBuffer,int bufsize)
 ...
 int adalm::readbuff(char *recvBuffer,int bufsize)
 {char data[1460];
...
int adalm::fir(char *a,int bw,int sample)
{
...
char c[1000];
 ...
 int adalm::WandR(char* data,char de,char ch,unsigned char art,bool w)
  {
string se;
if(w)se+="WRITE ";
else se+="READ ";
...
int adalm::opentransfer(unsigned long long *s,sockaddr_in *add,bool rec)
    {
...
if (rec)da[15]++;
  ...
  int adalm::sendun(unsigned long long *a,int m )
{int er=0;
    switch(m)
        {
       case 0:
...
int adalm::recany(unsigned long long *a,char * da )
{
    char len[10];
    int er=0;
    er=recv(*a,len,7,0);
...
int adalm::timeout(unsigned long long *a,int t)
{string s="TIMEOUT ";
s+=to_string(t);
s+="\r\n";
   int er=0;
...
/* Box-Muller: returns one sample (calls urand twice) */
