...
int S_comp(signed char *A,signed char *B, int len ,int N)
{char en[len];
 char s=0,ch=0;
...
int S_decomp(signed char *A,signed  char *B, int len ,int N)
{char en[len*2];
int ii=0,ra=0;
...
void short2byte(short *A,signed char* B,int len)
{
  float c=0;
  for(int i=0;i<len;i++)
  {
...
int intcontrol(char *ip)
{
 con = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    ...
    if (connect( con, (struct sockaddr*)&serverAddr, sizeof(serverAddr)) == -1) {
...
da=da+" 00000003 CYCLIC\r\n";
    ...
    // Connect to the server
    if (connect(wr, (struct sockaddr*)&serverAddr, sizeof(serverAddr)) == -1) {
        printf("Connection failed: %d\n", strerror(errno));
        close(wr);
        return -1;
    }

    if (send(wr, da.c_str(), da.length(), 0) == -1) {
...
da=da+" 00000003\r\n";
    ...
    // Connect to the server
    if (connect(re, (struct sockaddr*)&serverAddr, sizeof(serverAddr)) == -1) {
        printf("Connection failed: %d\n", strerror(errno));
        close(re);
        return -1;
    }

    if (send(re, da.c_str(), da.length(), 0) == -1) {
        printf("Send failed: %d\n", strerror(errno));
        close(re);
        return -1;
    }
...
char da[8],da1[80]={0};
  ...
  U=recv(con,da,3,0)  ;/////////////////////////////////////////***************problem here
...
int opentransfer(unsigned long long *s,sockaddr_in *add,bool rec)
    ...
    if (s[0] == -1) {
        ...
        return 1;
         ...
         if (connect(s[0], (struct sockaddr*)add, sizeof(add)) == -1) {
        printf("Connection failed: %d\n", strerror(errno));
        close(s[0]);
...
A.intcontrol("192.168.2.1");
    ...
    if (send(A.con, sendData, strlen(sendData), 0) == -1) {
        printf("Send failed: %d\n", strerror(errno));
        close(A.con);
...
serv_addr2.sin_port = htons(9000);
inet_pton(AF_INET, "127.0.0.1", &serv_addr2.sin_addr);

std::cout << "[SENDER] Trying to connect to remote server...\n";

while (1) {

    // Create a NEW socket each retry
    ...
    if (connect(loc, (sockaddr*)&serv_addr2, sizeof(serv_addr2)) == 0) {
        std::cout << "[SENDER] Connected!\n";
        break;
    }

    std::cout << "[SENDER] Remote server unreachable. Retrying...\n";
    close(loc);
    sleep(1);
}
...
//cout<<endl;
A.WandR("1024",3,0,0,0);
A.WandR("",3,0,0,0);

int su=com2re(f1,hh,32000);

cout<<A.readbuff(buf2,32000*4)<<endl;
...
//exit(1);
su=32000;
signed char ne[su],ll;
short2byte(&hh[0],&ne[0],su);

su=(su-(su%8));
cout<<"comp"<<su<<endl;
signed char com[su],com2[su];
int kj=S_comp(&ne[0],com, su ,8);
cout<<"comp"<<kj<<endl;
//for(int i=32000-100;i<32000;i++)cout<<i<<":"<<(short)ne[i]<<"  "<<endl;
//kj=S_decomp(com, com2, kj-1 ,8);
//f//or(int i=0;i<kj;i++)cout<<(short)ne[i]<<":"<<(short)com2[i]<<"  ";

cout<<endl;
for(int i=0;i<100;i++)cout<<(short)ne[i]<<"  "<<endl;
cout<<"comp"<<kj<<endl;
char *le=(char*)&kj;
send(loc,le,4,0);
   perror("ddd");
...
//A.writebuff(buf,32000*4);
 //auto E = chrono::steady_clock::now();
   com2reT(f1,hh,32000);
   short2byte(&hh[0],&ne[0],32000);

    kj=S_comp(&ne[0],com, 32000 ,8);
   cout<< kj<<endl;
